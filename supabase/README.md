...
1. Run [`migrations/20260805000000_frenchies_hr_pending_updates.sql`](migrations/20260805000000_frenchies_hr_pending_updates.sql) in the SQL Editor or via `supabase db push` (creates the shared desk state table and enables Realtime).
...
6. Also run the pending updates migration:
   - [`migrations/20260805000000_frenchies_hr_pending_updates.sql`](migrations/20260805000000_frenchies_hr_pending_updates.sql) — `kind`, `tips_to_pool`, `notes`, `desk_shared_state`, Realtime
...
| [`migrations/20260805000000_frenchies_hr_pending_updates.sql`](migrations/20260805000000_frenchies_hr_pending_updates.sql) | Adds `kind`, `tips_to_pool`, `notes`, `desk_shared_state`, Realtime |
...
```powershell
dir supabase\migrations\20260805000000_frenchies_hr_pending_updates.sql
...
3. Run the file below (copy all → paste → **Run**). Safe to run twice.

   | File | Adds |
   |------|------|
   | `supabase\migrations\20260805000000_frenchies_hr_pending_updates.sql` | `kind`, `tips_to_pool`, `notes`, `desk_shared_state`, Realtime |
